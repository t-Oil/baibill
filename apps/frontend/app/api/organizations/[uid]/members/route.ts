...
/**
 * GET handler for fetching organization members.
 * @param request NextRequest object
 * @param params Route params containing organization UID
 * @returns JSON response with members list
 */
export async function GET(request: NextRequest, props: { params: Promise<{ uid: string }> }) {
  ...
  const response = await fetch(`${API_URL}/api/organizations/${params.uid}/members`, {
    headers: {

...
/**
 * GET handler for fetching pending invitations for an organization.
 * @param request NextRequest object
 * @param params Route params containing organization UID
 * @returns JSON response with pending invitations list
 */
export async function GET(request: NextRequest, props: { params: Promise<{ uid: string }> }) {
  ...
  const response = await fetch(`${API_URL}/api/organizations/${params.uid}/invitations`, {
    headers: {
